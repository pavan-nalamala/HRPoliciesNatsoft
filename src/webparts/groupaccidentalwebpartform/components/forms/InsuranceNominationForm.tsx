...
// };
// type InsuranceNominationFormValues = {nomi
//   employeeName: string;
...
const InsuranceNominationForm = ({
  ...
  const validationSchema = Yup.object().shape({
    ...
    signature: Yup.mixed<File | string>()
      .required('Signature of Employee is required'),
  });
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const existingNominees = await sp.web.lists
          .getByTitle('PersonalAccidentalSchemeNominees')
        ...
        if (Array.isArray(values.nominees)) {
          ...
          for (const nominee of values.nominees) {
            ...
            const nomineePayload = stripNullishFields({
              Title: values.employeeName,
              ParentID: parentId,
              NomineeName: nominee.nomineeNameAndAddress,
              Relationship: nominee.relationship,
        ...
        alert(spMessage || fallback || 'Submission failed');
      }
    }
  });
    ...
    const loadExistingData = async (): Promise<void> => {
      ...
      try {
        ...
        const nomineeItems = await getListItemsByParentId(
          sp,
          'PersonalAccidentalSchemeNominees',
          parentId
        );
          ...
          <Card.Body className="p-4 p-md-5">
            <div className="text-center mb-4">
              <h3
                ...
                style={{ backgroundColor: '#f18200' }}
              >
                NOMINATION FORM FOR GROUP PERSONAL ACCIDENTAL SCHEME testing
              </h3>
            </div>
            ...
            <Form onSubmit={formik.handleSubmit}>
              ...
              <Row className="mt-4 g-3">
                ...
                <Col md={6} className="d-flex flex-column justify-content-end">
                  ...
                  <SignatureUpload
                    name="signature"
                    value={signaturePreview || formik.values.signature}
                    onChange={(file) => {
                      ...
                      onEmployeeSignatureChange?.(previewUrl);
                    }}
                    onBlur={formik.handleBlur}
                  />
                  {formik.touched.signature && formik.errors.signature && (
